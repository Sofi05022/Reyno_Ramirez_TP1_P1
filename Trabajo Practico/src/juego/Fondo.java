...
import java.awt.Color;
	...
	e.escribirTexto("❤️ " + mago.getVida(), menuRectX - menuRectAncho / 2 + 25, menuRectY -
	menuRectAlto / 2 + 40);
	// --- Mostrar la puntuación ---
	e.cambiarFont("Segoe UI Emoji", 16, Color.YELLOW); // Fuente para texto general
	e.escribirTexto("💰Puntos: " + puntuacion, menuRectX - menuRectAncho / 2 + 25, menuRectY -
	menuRectAlto / 2 + 63);
	// --- Mostrar los poderes como botones ---
	e.cambiarFont("Segoe UI Emoji", 16, Color.YELLOW); // Fuente para emojis
	e.escribirTexto("Poderes del mago🧙‍♂️", menuRectX - menuRectAncho / 2 + 25, menuRectY -
	menuRectAlto / 2 + 90);
	// Texto "Selecciona un poder" movido antes de los botones
	e.cambiarFont("Segoe UI Emoji", 14, Color.YELLOW);
	
	e.escribirTexto("Selecciona un poder", menuRectX - menuRectAncho / 2 + 25, menuRectY -
	menuRectAlto / 2 + 110);
	// Botón Bola de Fuego (movido a la derecha y ajustado Y)
	e.dibujarRectangulo(menuRectX - menuRectAncho / 2 + 75, menuRectY - menuRectAlto / 2 +
	135, 100, 20, 0, Color.GRAY);
	e.cambiarFont("Segoe UI Emoji", 12, Color.YELLOW); // Fuente para texto general
	e.escribirTexto("Bola de Fuego🔥", menuRectX - menuRectAncho / 2 + 30, menuRectY -
	menuRectAlto / 2 + 140);
	// Botón Fuerza (antes Escudo Mágico)
	e.dibujarRectangulo(menuRectX - menuRectAncho / 2 + 75, menuRectY - menuRectAlto / 2 +
	165, 100, 20, 0, Color.GRAY);
	e.cambiarFont("Segoe UI Emoji", 12, Color.YELLOW); // Fuente para texto general
	e.escribirTexto("Fuerza💪", menuRectX - menuRectAncho / 2 + 30, menuRectY - menuRectAlto /
	2 + 170); // Cambiado a "Fuerza"
	...
	if (poderActivoNombre != null && usosPoderRestantes > 0) {
		...
		e.escribirTexto("Activo: " + (poderActivoNombre.equals("bola") ? "🔥" : "💪"), menuRectX -
		menuRectAncho / 2 + 25, menuRectY - menuRectAlto / 2 + 195); // Cambiado a "Fuerza"
		e.escribirTexto("Usos: " + usosPoderRestantes, menuRectX - menuRectAncho / 2 + 25,
		menuRectY - menuRectAlto / 2 + 215);
	}
	// Mostrar demonios restantes (Y ajustado)
	e.cambiarFont("Segoe UI Emoji", 16, Color.RED); // Fuente para emojis
	e.escribirTexto("👿 " + demoniosRestantes, menuRectX - menuRectAncho / 2 + 25, menuRectY -
	menuRectAlto / 2 + 245);
	}
